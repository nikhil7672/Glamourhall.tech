...
// Replace this with your actual Browserless API key
  ...
  try {
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        // Give some extra time before scraping
        await sleep(2000);

        // Get the page content
        const html = await page.content();

        // Parse the content using Cheerio
        products = parseMyntra(html);

        // If scraping was successful, break the retry loop
        break;
