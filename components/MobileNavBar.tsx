...
export default function MobileNavBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden">
      <div className="w-full bg-gradient-to-t from-white/95 via-white/80 to-transparent dark:from-gray-900/95 dark:via-gray-900/80 dark:to-transparent backdrop-blur-md rounded-t-xl shadow-2xl flex justify-around items-center py-1 border-t border-gray-200 dark:border-gray-700">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = pathname?.startsWith(item.path);
          const activeClass = isActive 
            ? `${activeStyles[item.path as keyof typeof activeStyles]} shadow-md transform scale-105`
          ...
          return (
            <Link
              key={item.path}
              href={item.path}
              className="relative flex flex-col items-center justify-center px-2 pt-1 transition-all duration-200"
            >
              ...
              <span className={`mt-0.5 text-[12px] font-medium transition-colors duration-200 ${isActive ? 'text-current' : 'text-gray-500 dark:text-gray-400'}`}>
                {item.label}
              </span>
            </Link>
          );
